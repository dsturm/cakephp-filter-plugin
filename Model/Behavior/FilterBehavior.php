				...
				if (strpos($field, '.')) {
					...
					if (array_key_exists($associatedModel, $associated)) {
						...
						if ($associated[$associatedModel] == 'hasAndBelongsToMany') {
							...
							$condition = $model->{$associatedModel}->find('all', array('fields' => 'DISTINCT id', 'conditions' => $field . ' ' . $search_value, 'recursive' => -1, 'callbacks' => false // because otherwise this `beforeFind` would be called again
							));
							...
							$result = $model->{$associatedModel}->{$assoc['with']}->find('all', array('fields' => 'DISTINCT ' . $assoc['foreignKey'], 'conditions' => array('OR' => $conditions), 'recursive' => -1, 'callbacks' => false // because otherwise this `beforeFind` would be called again
							));
							...
							$result = $model->{$associatedModel}->find('all', array('fields' => 'DISTINCT ' . $assoc['foreignKey'], 'conditions' => array('OR' => $conditions), 'recursive' => -1, 'callbacks' => false // because otherwise this `beforeFind` would be called again
							));
...
/* end of file FilterBehavior.php */
